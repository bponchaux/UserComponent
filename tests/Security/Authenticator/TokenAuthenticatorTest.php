...
use Biig\Component\Infrastructure\Hexagonal\DoctrineOrmRepository;
use Biig\Component\User\Security\Authenticator\TokenAuthenticator;
...
class TokenAuthenticatorTest extends TestCase
{
    ...
    {
        $this->userRepository = $this->prophesize(UserTokenProviderInterface::class);
    ...
    public function testGetUser()
    {
        ...
        $this->userRepository->loadUserByConfirmationToken('foobar')->shouldBeCalled();
    ...
    {
        $server = ['REQUEST_URI' => 'reset-password', 'REQUEST_METHOD' => 'PUT'];
        $request = new Request([], [], [], [], [], $server, null);
        $result = $this->authenticator->supports($request);
        $this->assertTrue($result);

        $server = ['REQUEST_URI' => 'reset-password/token', 'REQUEST_METHOD' => 'PUT'];
        $request = new Request([], [], [], [], [], $server, null);
        $result = $this->authenticator->supports($request);
        $this->assertTrue($result);
        ...
        $server = ['REQUEST_URI' => 'bad-request', 'REQUEST_METHOD' => 'PUT'];
        $request = new Request([], [], [], [], [], $server, null);
        $result = $this->authenticator->supports($request);
        $this->assertFalse($result);
    }
